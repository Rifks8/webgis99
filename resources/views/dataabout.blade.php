    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                @section('subtitle', 'DATA PESAN NI BOS')
                <div class="p-6">
                    <div class="flex items-center">
                        <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" class="w-8 h-8 text-gray-400"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold">Data Pesan</div>
                    </div>
                    ...
                    <div class="ml-12">
                        ...
                        <button class="mb-8 bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick="window.location.reload();">Refresh Page</button>
                        ...
                        <table class="border-collapse border border-green-800 table-fixed">
                            <thead>
                                <tr>
                                    ...
                                    <th class="w-1/2 border-4 border-black bg-gray-300">Email</th>
                                    <th class="w-1/2 border-4 border-black bg-gray-300">Pesan</th>                                
                            ...
                            @foreach( $pesan as $pesan )
                                <tr>
                                    <th class="border-4 border-green-600 text-lg">{{ $loop -> iteration }}</th>
                                    <td class="border-4 border-red-600 text-lg">{{ $pesan -> nama }}</td>
                                    <td class="border-4 border-blue-600">{{ $pesan -> email }}</td>
                                    <td class="border-4 border-pink-600">{{ $pesan -> isipesan }}</td>
                                    <td class="border-4 border-yellow-600">
                                        <a href="mailto:{{$pesan -> email}}?subject=Balas%20Pesan%20Website%20SIG%20Tangsel">
                                            <button class="bg-blue-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Balas</button>
                                        </a>
                                        <form action="/dataabout/{{ $pesan -> id }}" method="POST">
                                        @method('delete')
                                        @csrf
